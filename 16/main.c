...
int solve(char **m, size_t m_len, size_t row_size, int y, int x, int dir) {
  int res = 0;
  ...
  beams[0] = (Beam){.dir = dir, .y = y, .x = x, .to_remove = false};
...
int main() {
  ...
  for (int col = 0; col < row_size; col++) {
    int from_top = solve(m, m_len, row_size, -1, col, 1);
    int from_bottom = solve(m, m_len, row_size, m_len, col, 3);
    printf("from_top: %d\n", from_top);
    printf("from_bottom: %d\n", from_bottom);
    res = from_top > from_bottom && from_top > res ? from_top : from_bottom > from_top && from_bottom > res ? from_bottom : res;
  }
  for (int row = 0; row < m_len; row++) {
    int from_left = solve(m, m_len, row_size, row, -1, 0);
    int from_right = solve(m, m_len, row_size, row, row_size, 2);
    printf("from_left: %d\n", from_left);
    printf("from_right: %d\n", from_right);
    res = from_left > from_right && from_left > res ? from_left : from_right > from_left && from_right > res ? from_right : res;
  }
